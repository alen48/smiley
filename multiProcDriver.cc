...
int loCriticality = 1;
int hyperPeriod;
vector<int> processorTasks;
vector<int> periods;
int noOfLoTasks;
multimap<pair<double, double>, Task> orderedHiTasks;
vector<Task> LoTasks;
Task *LoTaskset;
priority_queue<Job> LoJobQ;
...
mutex loQAddMutex;
mutex slackMutex;
mutex slackFinishMutex;
mutex minSlackMutex1;
mutex minSlackMutex2;
mutex printMutex;
int loopCount1 = 0;
int loopCount2 = 0;
vector<int> decProc;
vector<int> decProcNext;
int decProcCount = 0;
...
checkProcessor(void)
{
	...
	int q = 0;
	int j;
	for (multimap<pair<double, double>, Task>::iterator it = orderedHiTasks.begin(); it != orderedHiTasks.end() ; it++) {
...
allocateTasks(void)
{
	...
	for (vector<Task>::iterator it = LoTasks.begin(); it != LoTasks.end() ; it++) {
...
generateLoJobs(void)
{
	...
	double
calcSlack(Task *taskset, int noOfEdfTasks, double currTime, Job loQTop, priority_queue<Job> readyQ, int procNo) 
{
	double slack = 0.0;
	...
	while (!readyQ.empty()) {	
		...
		// 	cout << "LO_";
		// cout << readyQ.top().taskNo << readyQ.top().jobNo << " ";
		readyQ.pop();
	}			
	cout << "\n";
	for(int i = 0; i < noOfEdfTasks; i++) {
	...
	// printMutex.lock();
	// cout << "current time: " << currTime << "; loDeadline: " << loQTop.deadline << "; maxD: " << maxDeadline << "; incoming task : J_LO_" << loQTop.taskNo << loQTop.jobNo << "\n";
	// for (vector<Job>::iterator i = type1.begin(); i != type1.end(); i++) {
	// 	if (i->jobCriticality < sysCriticality)
	// 		cout << "LO_";
	// 	cout << i->taskNo << i->jobNo << "\n";
	// }
	// printMutex.unlock();
	...
	for(int i = 0; i < (int) type1.size(); i++) {
		...
		if (temp.deadline > dynamicT) {
			if (temp.deadline > maxDeadline) {
				...
				// cout << temp.taskNo << temp.jobNo << " " << temp.remainingT * (maxDeadline - temp.arrivalT) * utilisation / (temp.deadline - temp.arrivalT) << "; ";
			} else{
...
{	
	...
	string fileName("inputP");
	string outputFile("outputP");	
	string buffer = to_string(procNo);	
	fileName = fileName + buffer;
